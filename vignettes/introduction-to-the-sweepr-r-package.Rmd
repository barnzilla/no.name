...
```{r warning = FALSE, message = FALSE, eval = FALSE}
utils::install.packages("https://github.com/barnzilla/r-package-sources/raw/master/sweepr_0.11.6.tar.gz", repos = NULL, type = "source")
```
...
```{r}
parameter_sweep_size <- try.various.parms.values(
  baseline_results,
...
dim(parameter_sweep_size$parms.to.try)
```
...
You can render a scatter plot of any two vectors in the `outcomes_summary_df` data frame.
...
You can also render a tornado plot of any vector within the `outcomes_summary_df` data frame.
...
You can also render a tornado table, which tabulates all of the values from the tornado plot.

```{r}
get_tornado_table(outcome_variable = "maxInc", parameters = parameters_swept, outcomes = outcomes_summary_df)
```


## Demo data

Example data from a parameter sweep comes with the `sweepr` R package. You can use this data to examine the structure of the output, or to test some of the functions in the `sweepr` R package (e.g., `get_tornado_plot`).
...
parameters_swept <- sweepr_demo$parameters
outcomes_summary_df <- sweepr_demo$results
