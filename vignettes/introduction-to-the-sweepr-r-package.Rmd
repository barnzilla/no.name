...
output: 
  rmarkdown::html_vignette:
    ...
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Introduction to the sweepr package}
...
```{r warning = FALSE, message = FALSE, eval = FALSE}
utils::install.packages("https://github.com/barnzilla/r-package-sources/raw/master/sweepr_0.11.3.tar.gz", repos = NULL, type = "source")
```
...
parms.tried.df <- sweepr_demo$parameters
```

```{r}
get_scatter_plot(outcomes_summary_df$delta.overwrite, outcomes_summary_df$maxInc)
```

You can also render a tornado plot of any vector within the `outcomes_summary_df` dataframe.

```{r}
get_tornado_plot("maxInc")
```
